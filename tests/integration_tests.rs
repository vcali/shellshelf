...
fn test_help_output() {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("A CLI tool for managing curl commands"))
...
fn test_add_command() {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("Added curl command: curl https://example.com/test"));
...
fn test_search_no_results() {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("No curl commands found matching keywords: nonexistent"));
}
...
fn test_multiple_keyword_search() {
    ...
    let mut cmd = Command::cargo_bin("reqbib").unwrap();
    cmd.env("HOME", temp_dir.path());
    cmd.arg("--add").arg("curl -X POST https://api.github.com/repos");
    cmd.assert().success();
...
curl https://httpbin.org/get"#;
    ...
    cmd.assert()
        ...
        .stdout(predicate::str::contains("curl -X POST https://github.com/api/repos"))
