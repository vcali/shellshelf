...
mod tests {
    ...
    fn test_curl_command_new() {
        let command = "curl -X POST https://api.example.com/users -H 'Content-Type: application/json'".to_string();
    ...
    fn test_curl_database_search() {
        ...
        // Search by domain
        let results = db.search(&vec!["github".to_string()]);
        assert_eq!(results.len(), 2);
        
        // Search by path
        let results = db.search(&vec!["users".to_string()]);
        assert_eq!(results.len(), 1);
        
        // Search by multiple keywords
        let results = db.search(&vec!["api".to_string(), "POST".to_string()]);
        assert_eq!(results.len(), 1);
        
        // Search with no matches
        let results = db.search(&vec!["nonexistent".to_string()]);
...
curl -H "Authorization: Bearer token" https://api.example.com/data"#;
        ...
        assert!(commands.contains(&"curl -H \"Authorization: Bearer token\" https://api.example.com/data".to_string()));
    }
...
: 1647875050:0;curl   -H "Content-Type: application/json" https://httpbin.org/post"#;
        ...
        assert!(commands.contains(&"curl https://example.com".to_string()));
        assert!(commands.contains(&"curl -X POST https://api.github.com/repos".to_string()));
        assert!(commands.contains(&"curl   -H \"Content-Type: application/json\" https://httpbin.org/post".to_string()));
    }
...
: 1647875010:0;curl -H "Auth: token" https://example4.com"#;
    ...
    fn test_search_partial_keyword_match() {
        ...
        let results = db.search(&vec!["repo".to_string()]);
        assert_eq!(results.len(), 1);
        
        let results = db.search(&vec!["hub".to_string()]);
        assert_eq!(results.len(), 1);
    }
}
